  ...
  return (
    <Popover>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 min-w-[16px] h-4 px-1 rounded-full bg-primary text-white text-[10px] font-mobile font-semibold flex items-center justify-center leading-none">
            {unreadCount}
          </span>
        )}
      </PopoverTrigger>
      <PopoverContent
        align="end"
        sideOffset={8}
        className="w-[380px] p-0 bg-white border border-border shadow-lg rounded-2xl overflow-hidden"
      >
        <header className="flex items-center justify-between px-4 py-3 border-b border-border">
          <div className="flex items-center gap-2">
            <h3 className="font-mobile font-semibold text-sm text-foreground">Notifications</h3>
            {unreadCount > 0 && (
              <span className="text-[11px] font-mobile font-semibold text-primary bg-primary/10 border border-primary/15 rounded-full px-1.5 leading-tight py-0.5">
                {unreadCount} new
              </span>
            )}
          </div>
        ...
        <footer className="border-t border-border">
          <Link
            to="/m/notifications"
            className="flex items-center justify-center gap-1 py-2.5 text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          >
            See all
...
function Row({ n, onClick }: { n: Notification; onClick?: () => void }) {
  ...
  const inner = (
    <div className={cn("flex items-start gap-3 px-4 py-3 transition-colors", n.unread ? "bg-primary/40 hover:bg-primary/10" : "hover:bg-muted")}>
      {n.avatar ? (
        <div className="relative shrink-0">
          <img src={n.avatar} alt="" className="w-9 h-9 rounded-full object-cover" />
          <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-white flex items-center justify-center border border-border">
