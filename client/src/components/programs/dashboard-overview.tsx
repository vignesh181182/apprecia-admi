    ...
    <div className={cn("space-y-4", variant === "web" && "pt-3")}>
      ...
      <section className="bg-white rounded-2xl border border-border p-4 md:p-5">
        <header className="mb-3">
          <h3 className="font-mobile font-semibold text-foreground">Budget Progress</h3>
        </header>
        <div className="flex items-baseline justify-between gap-2 mb-2">
          ...
          <span className="text-sm font-mobile font-semibold text-muted-foreground tabular-nums">
            {usedPct}%
          </span>
        </div>
        ...
        {program.prizes && program.prizes.length > 0 && (
          <div className="grid grid-cols-3 gap-2 mt-4">
            {program.prizes.map((p) => (
              ...
              >
                <p className="font-mobile text-base font-semibold text-foreground">
                  {currency}
                  {formatK(p.amount)}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">{ordinal(p.rank)} prize</p>
              </div>
            ))}
          </div>
        )}
      ...
      {program.programLeaderboard && program.programLeaderboard.length > 0 && (
        <section className="bg-white rounded-2xl border border-border p-4 md:p-5">
          <header className="flex items-center justify-between mb-3">
            <h3 className="font-mobile font-semibold text-foreground">Leaderboard</h3>
            <TrendingUp className="w-4 h-4 text-muted-foreground" />
          </header>
          <ul className="divide-y divide-border">
            {program.programLeaderboard.map((row) => (
              <li key={row.rank} className="flex items-center gap-3 py-2.5">
                <span className="w-6 h-6 rounded-full bg-muted text-muted-foreground text-xs font-mobile font-semibold flex items-center justify-center shrink-0">
                  {row.rank}
                </span>
                <img
                  src={row.avatar}
                ...
                <p className="font-mobile font-semibold text-foreground shrink-0 tabular-nums">
                  {row.points}{" "}
          ...
          <button className="text-xs text-muted-foreground hover:text-foreground mt-1">View all</button>
        </section>
      )}

      {program.attentionItems && program.attentionItems.length > 0 && (
        <section className="bg-white rounded-2xl border border-border p-4 md:p-5">
          <h3 className="font-mobile font-semibold text-foreground mb-2">Needs Attention</h3>
          <ul className="space-y-3">
            {program.attentionItems.map((item, i) => (
              <li key={i} className="flex items-start gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-mobile font-semibold text-foreground leading-snug">
                    {item.title}
                  </p>
