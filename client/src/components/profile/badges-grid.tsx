  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      {badges.map((b, i) => (
        <div
          key={`${b.badge}-${i}`}
          className="bg-white rounded-2xl border border-border p-4 flex flex-col items-center text-center"
        >
          <img src={BADGE_IMAGE[b.badge]} alt={b.label} className="w-24 h-24 object-contain" />
          <p className="mt-2 font-mobile font-semibold text-primary text-sm">{b.label}</p>
          <p className="text-xs text-muted-foreground mt-0.5">From {b.earnedFrom}</p>
